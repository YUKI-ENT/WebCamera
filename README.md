...
GUI の `Device auth enabled` を有効にすると、登録済み端末だけがアップロードできます。`Create registration QR` で5分間・1回限りの登録QRを発行し、スマホで読み取って端末登録します。登録済み端末は `Device Tokens` 一覧から削除できます。端末トークンは `devices.json` にハッシュ保存されます。
